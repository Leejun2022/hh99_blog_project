...
router.get("/posts/like", authMiddleware, async (req, res, next) => {
  try {
    ...
    const likes = await postLikes.findAll({ where: { userId: user.userId } });
    ...
    const posts = await Posts.findAll({ where: { postId: likePostIds } });
...
router.put("/posts/:postId/like", authMiddleware, async (req, res, next) => {
  try {
    ...
    let likes = await postLikes.findAll({ where: { postId } });
